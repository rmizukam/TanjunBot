...
async def description(
    ...
    await ctx.edit_initial_response(
            content="Set Description for embed:",
            components=[]
        )
    event = await collect_response(
            ctx,
            validator=lambda _,
            event: len(event.message.content) < 4096
        )
...
async def change_icon(
    ...
    await ctx.edit_initial_response(
            content="Set Icon for embed:",
            components=[]
        )
    event = await collect_response(
            ctx,
            validator=lambda _,
            event: event.message.content.startswith("http")
        )
...
async def image(
    ...
    await ctx.edit_initial_response(
                content="Set Image for embed:",
                components=[]
            )
    event = await collect_response(
            ctx,
            validator=lambda _,
            event: len(event.message.content) < 2000
        )
...
async def footer(
    ...
    client.metadata['embed'].set_footer(
            text=event.content,
            icon=guild.icon_url
        )
    await ctx.edit_initial_response(
...
async def add_field(
    ...
    field_inline = False
    await ctx.edit_initial_response(
            "What would you like the Field Title to be?",
            components=[]
        )
    name_event = await collect_response(
            ctx,
            validator=lambda _,
            event: len(event.message.content) < 256
        )
    field_name = name_event.content
    await ctx.edit_initial_response(
            "What would you like the Field Body to be?",
            components=[]
        )
    ...
    await ctx.edit_initial_response(
            "Would you like this to be inline? (y/n)",
            components=[]
        )
    inline_event = await collect_response(
            ctx,
            validator=['yes', 'y', 'ye', 't', 'true', 'no', 'n', 'false', 'f']
        )
    if inline_event.content.lower() in ['yes', 'y', 'ye', 't', 'true']:
        ...
        await ctx.edit_initial_response(
                "Didn't understand that answer, assuming `No`.",
                embed=None,
                components=[]
            )
...
async def change_field(
    ...
    await ctx.edit_initial_response(
            f'''What Field would you like to update?
                Provide just the Index Number: {nl} {fields}''',
            components=[]
        )
...
async def change_text_on_publish(
    ...
    await ctx.edit_initial_response(
            content="Set Text for Publish:",
            components=[]
        )
    event = await collect_response(
            ctx,
            validator=lambda ctx,
            event: len(event.content) < 2000
        )
...
async def add_ping(
    ...
    while True:
        ...
        try:
            async with bot.stream(GuildMessageCreateEvent,
                                  timeout=60).filter((
                ...
                async for event in stream:
                    ...
                    for role in roles:
                        if (role.id == event.content
                                or role.name == event.content):
                            found_role = role
                            break

                    if not found_role:
                        await ctx.edit_initial_response(
                                content=f'''Role `{event.content}`
                                             not found! Try again?'''
                            )
                    else:
                        client.metadata['roles'].append(role)
                        await ctx.edit_initial_response(
                                content=f'''Added {found_role.mention}
                                 to ping list for Publish!'''
                            )

                    await event.message.delete()
                    await asyncio.sleep(5)
                    await ctx.edit_initial_response(
                        content='''Add Ping for Publish. To return to menu,
                                    send ❌.''',
                        components=[]
                        )
        except asyncio.TimeoutError:
...
async def show_status(
    ...
    status_txt = (
            f'''Text to show with this embed on publish:
                 ```{client.metadata['text']}```'''
            "\n"
...
async def publish_to_channel(
    ...
    await ctx.edit_initial_response(
            content="What channel would you like to Publish to:",
            components=[]
        )
